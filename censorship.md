...
# Tornado Cash: Censorship in Blockchain Naming Systems

Most blockchain naming systems claim to be "censorship resistant":
there exists no central authority that can disrupt or modify the
resolution of a name.  As a consequence these systems also usually
lack a means of recovering from various errors (such as long or stolen
cryptographic keys) as such recovery mechanisms generally rely on the
presence of the same sort of central authority that can censor
responses.

Unfortunately practice has shown that although these systems may claim
to be censorship resistant due to the lack of central authorities,
there actually exist significant central intermediaries that can and
do censor blockchain-based naming schemes.  The most significant
example of such censorship involves Tornado Cash.

Tornado Cash is an autonomous Ethereum program designed to act as an
automatic money laundering system.  In Tornado Cash a user deposits a
fixed amount into a pool (most commonly .1, 1, 10, or 100 Ethereum,
which is currently worth roughly $160, $1600, $16,000 and $160,000).
Sometime later the same user can perform a withdraw of the same amount
minus a transaction fee.

In order to obfucate the linkage between the deposit and the withdraw
the withdraw is conducted using a zero knowledge proof scheme: the
withdraw contains a proof that the corresponding deposit was not
previously withdrawn but, critically, ***does not*** identify the
particular deposit.

Tornado Cash naturally attracted a lot of criminal behavior for its
automated money laundering.  In 2022, North Korean hackers stole
roughly $600M in Ethereum which they started laundering through
Tornado Cash.  In response the Office of Foreign Assets Control (OFAC)
sanctioned Tornado Cash.

The response by Ethereum infrastructure providers was swift.  Infura
in particular stopped resolving Tornado Cash related Ethereum
addresses.  Similarly, the `eth.link` ENS service (which combines a
DNS resolving service and Cloudflare's IPFS gateway) initially blocked
both `tornadocash.eth` and `tornadocashcomunity.eth` (as I write this
they have unblocked `tornadocash.eth`).  Similarly, the `eth.limo`
service is currently blocking `tornadocash.eth` but allowing
`tornadocashcommunity.eth`.
